...
class CatBoost:
    ...
    def __init__(self, pca: bool):
        ...
        self.save_path_philips = (
            "cat_philips_total_train_pca.pkl" if pca else "cat_philips_total_train.pkl"
        )
        ...
        try:
            with open(self.save_path, "rb") as f:
                ...
                self.model = CatBoostClassifier(**best_params_)
        except:
    ...
    def siemens(self):
        ...
        for train_idx, test_idx in self.kfold.split(
            ...
            accuracy = np.round(accuracy_score(y_test, fold_pred), 4)
            print(f"{n_iter}-trial 교차검증 정확도 : {accuracy}, 학습 정확도 : {fold_pred_train}")
            ...
            sens, spec = tper[idx], 1 - fper[idx]
            print(
                "%d-Trial Best threshold = %.3f, Sensitivity = %.3f, Specificity = %.3f"
                % (n_iter, best_threshold, sens, spec)
            )
